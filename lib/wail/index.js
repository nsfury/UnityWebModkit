...
class WailParser extends BufferReader {
    ...
    addImportEntry(options) {
        ...
        switch (convertedKind) {
            ...
            case KIND_GLOBAL:
                ...
                if (options.mutability === 0 ||
                    options.mutability === 1 ||
                    options.mutability === true ||
                    options.mutability === false) {
    ...
    addExportEntry(index, options) {
        const newEntry = {};
        if (typeof options.fieldStr == "string" ||
            options.fieldStr instanceof String) {
    ...
    _readSection() {
        ...
        if (id != SECTION_DATACOUNT) {
            ...
            for (let missingId = 0; missingId < id; missingId++) {
                ...
                if (thisSectionRequired &&
                    !(thisSectionRequired & this._parsedSections)) {
                    switch (missingId) {
                        case SECTION_TYPE:
    ...
    _parseTypeSection() {
        ...
        if (!window.Modkit || !window.Modkit.internalWasmTypes) {
            populateThisTimeOnly = true;
            window.Modkit = window.Modkit || {};
            window.Modkit.internalWasmTypes = [];
        }
        ...
        for (let typeIndex = 0; typeIndex < oldCount; typeIndex++) {
            ...
            if (!populateThisTimeOnly)
                continue;
            window.Modkit.internalWasmTypes.push({
    ...
    _parseFunctionSection() {
        ...
        const reader = new BufferReader(oldPayload);
        let populateThisTimeOnly = false;
        if (!window.Modkit || !window.Modkit.internalWasmFunctions) {
            populateThisTimeOnly = true;
            window.Modkit = window.Modkit || {};
            window.Modkit.internalWasmFunctions = [];
        }
        for (let funcIndex = 0; funcIndex < oldCount; funcIndex++) {
            ...
            if (!populateThisTimeOnly)
                continue;
            window.Modkit.internalWasmFunctions.push({
                funcType,
            });
        }
        let newCount = oldCount;
    ...
    _parseElementSection() {
        ...
        let populateThisTimeOnly = false;
        if (!window.Modkit || !window.Modkit.internalMappings) {
            populateThisTimeOnly = true;
            window.Modkit = window.Modkit || {};
            window.Modkit.internalMappings = [];
        }
        for (let elemIndex = 0; elemIndex < oldCount; elemIndex++) {
            ...
            if (!populateThisTimeOnly)
                continue;
            window.Modkit.internalMappings.push({
                index: memIndex,
                elements,
            });
        }
